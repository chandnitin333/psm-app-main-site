...
@Component({
  selector: 'app-ward-wise-pinyache-pani-list',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './ward-wise-pinyache-pani-list.component.html',
  styleUrl: './ward-wise-pinyache-pani-list.component.css'
})
export class WardWisePinyachePaniListComponent {
  receivedData : any;
  adharList: any;
  ward_number: any;
...
get_adhar_ward_wise_list(){
  ...
  downloadPDFMobile() {
    ...
    const fileName = `pinyache_pani_${currentDate}.pdf`;
  ...
  downloadPDFDirect() {
    ...
    setTimeout(() => {
      ...
      const seconds = String(now.getSeconds()).padStart(2, '0');
      const currentDate = `${day}-${month}-${year}_${hours}-${minutes}-${seconds}`;
      const fileName = `pinyache_pani_list_${currentDate}.pdf`;
