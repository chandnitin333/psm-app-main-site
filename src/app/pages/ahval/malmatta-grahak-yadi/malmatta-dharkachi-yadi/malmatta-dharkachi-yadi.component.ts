...
export class MalmattaDharkachiYadiComponent {
  ...
  constructor(private router: Router, private grahakYadiService: MalmattaGrahakYadiService, private route: ActivatedRoute, private spinner: LoaderService, private toastr: ToastrService,
   ...
   ngOnInit() {
      if (this.isPublic) {
        this.get_public_malmatta_dharkachi_yadi_list();
      } else {
  ...
  get_public_malmatta_dharkachi_yadi_list(){
    this.spinner.show();
      ...
      next: (res: any) => {
        try {
          if (res?.status === 200 && res?.data) {
            ...
            if (this.malmattaDarkList?.yearRs10 && this.malmattaDarkList.yearRs10.length > 0) {
    ...
    this.grahakYadiService.malmattaDarkachiYadi(param).subscribe({
      next: (res: any) => {
        try {
          ...
          if (!this.malmattaDarkList?.rs3 || this.malmattaDarkList.rs3.length === 0) {
  ...
  private buildPerRecordQrLinks(param: any): void {
    ...
    const rows: any[] = this.malmattaDarkList?.rs3 || [];
...
downloadPDFMobile() {
      ...
      const fileName = `malamtta_dharkachi_yadi_${currentDate}.pdf`;
    ...
    async downloadPDFDirect() {
      ...
      setTimeout(async () => {
        try {
          ...
          const seconds = String(now.getSeconds()).padStart(2, '0');
          const currentDate = `${day}-${month}-${year}_${hours}-${minutes}-${seconds}`;
          const fileName = `malmatta_dharkachi_yadi_${currentDate}.pdf`;
